...
import { mapTeamName } from './teamMapping.js'
...
// Consulta API-Football (fuente principal, gratis y sin límite de tokens)
async function fetchResultsFromAPIFootball(pendingMatches) {
  if (pendingMatches.length === 0) return null

  // Determinar las fechas UTC involucradas (puede haber partidos a la noche que caen en el día siguiente UTC)
  const dates = new Set()
  pendingMatches.forEach(m => {
    if (m.kickoff) dates.add(m.kickoff.slice(0, 10)) // YYYY-MM-DD en UTC
  })

  const allFixtures = []
  for (const date of dates) {
    const resp = await fetch(`/api/results?date=${date}`)
    const data = await resp.json()
    if (data.error) throw new Error(`API-Football error: ${JSON.stringify(data.error)}`)
  ...
  for (const m of pendingMatches) {
    const fixture = allFixtures.find(f => {
      const home = mapTeamName(f.home)
      const away = mapTeamName(f.away)
      return home === m.home && away === m.away
    })
    if (fixture && FINISHED_STATUSES.has(fixture.status) && fixture.homeGoals !== null && fixture.awayGoals !== null) {
      results[m.id] = { home: fixture.homeGoals, away: fixture.awayGoals }
    }
  }
  return { results, source: 'API-Football' }
}
...
export default function App() {
  ...
  async function runAutoSync() {
    ...
    try {
      ...
      // 1. Fuente principal: API-Football (gratis, sin límite de tokens)
      try {
        const apiData = await fetchResultsFromAPIFootball(pending)
        if (apiData && Object.keys(apiData.results).length > 0) {
          combined = { ...combined, ...apiData.results }
          sources.push(apiData.source)
        }
      } catch (e) {
        console.error('Error API-Football:', e)
      }

      // 2. Respaldo: IA con web search, solo para los que API-Football no resolvió
      const stillPending = pending.filter(m => !(m.id in combined))
      if (stillPending.length > 0) {
        try {
          const aiData = await fetchResultsFromAI(stillPending)
          if (aiData && Object.keys(aiData.results).length > 0) {
            combined = { ...combined, ...aiData.results }
            sources.push(aiData.source || 'Claude AI')
          }
        } catch (e) {
          console.error('Error IA fallback:', e)
        }
      }
      ...
      const merged = { ...(official.results || {}), ...combined }
