...
// Busca resultados solo de partidos que terminaron hace menos de 3 horas
async function fetchRecentResultsFromAI() {
  const now = Date.now()
  const recentMatches = MATCHES.filter(m => {
    if (!m.kickoff) return false
    const kickoffMs = new Date(m.kickoff).getTime()
    const endMs = kickoffMs + 110 * 60 * 1000 // partido + tiempo extra estimado
    return now > endMs && now < endMs + 3 * 60 * 60 * 1000 // terminó hace menos de 3hs
  })
  ...
  const response = await fetch('/api/claude', {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: `Buscá el resultado final de estos partidos del Mundial 2026 que acaban de terminar:\n${matchList}\n\nRespondé SOLO con JSON sin markdown:\n{"results": {"1": {"home": 2, "away": 0}}, "source": "fuente"}\nSolo incluí los que ya terminaron con resultado definitivo.`
      }],
    }),
  })
  const data = await response.json()
...
function MatchCard({ match, predictions, officialResults, setPred, S, showGroup = false }) {
  ...
  async function getAIPrediction() {
    if (!ANTHROPIC_KEY) { setAiSuggestion({ error: 'Falta la API key de Anthropic' }); return }
    setAiLoading(true)
...
{"home": 2, "away": 1, "reasoning": "explicación breve en español de máximo 2 oraciones"}`
      ...
      setAiSuggestion({ error: 'No se pudo obtener el pronóstico' })
    }
    setAiLoading(false)
  }
...
export default function App() {
  ...
  // Load official results on mount + auto-sync hourly + real-time every 15min
  useEffect(() => {
    loadOfficial()
    const hourlyInterval = setInterval(runAutoSync, AUTO_SYNC_INTERVAL)
    // Check for recent results every 15 minutes
    const realtimeInterval = setInterval(runRealtimeSync, 15 * 60 * 1000)
    return () => { clearInterval(hourlyInterval); clearInterval(realtimeInterval) }
  }, [])
  ...
  async function runRealtimeSync() {
    if (!ANTHROPIC_KEY || isSyncing) return
    try {
      const recent = await fetchRecentResultsFromAI()
      if (!recent || !recent.results || Object.keys(recent.results).length === 0) return
      ...
      console.error('Error en realtime sync:', e)
    }
  }
